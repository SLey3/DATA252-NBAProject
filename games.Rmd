...
results <- data.frame(
  ...
  RMSE_CV  = c(mean(cv_poly1_rmse), mean(cv_poly2_rmse), mean(cv_poly3_rmse),
               mean(cv_loess_rmse, na.rm = TRUE), mean(cv_gam_rmse)),
  SD_CV    = c(sd(cv_poly1_rmse), sd(cv_poly2_rmse), sd(cv_poly3_rmse),
               sd(cv_loess_rmse, na.rm = TRUE), sd(cv_gam_rmse))
) %>%
  mutate(across(where(is.numeric), ~ round(.x, 4))) %>%
  arrange(RMSE_CV)

cv_gam_multi_rmse <- sapply(folds, function(idx) {
  train_data <- teams_winners[-idx, ]
  test_data  <- teams_winners[idx, ]
  fit <- gam(point_diff ~ fieldGoalsPercentage + assists + turnovers + reboundsTotal + steals + pointsInThePaint + freeThrowsPercentage, data = train_data)
  preds <- predict(fit, newdata = test_data)
