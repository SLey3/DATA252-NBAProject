...
```{r}
teams_clean <- teams_winners[, c("point_diff", "fieldGoalsPercentage", "conference")]
...
parallel_cv <- train(point_diff ~ fieldGoalsPercentage + conference, 
                     data = teams_clean, method = "lm", trControl = ctrl)

interaction_cv <- train(point_diff ~ fieldGoalsPercentage * conference, 
                        data = teams_clean, method = "lm", trControl = ctrl)

simple_mod      <- lm(point_diff ~ fieldGoalsPercentage, data = teams_clean)
parallel_mod    <- lm(point_diff ~ fieldGoalsPercentage + conference, data = teams_clean)
interaction_mod <- lm(point_diff ~ fieldGoalsPercentage * conference, data = teams_clean)
...
ggplot(teams_winners, aes(x = teamCity, y = point_diff)) +
  geom_boxplot() + 
