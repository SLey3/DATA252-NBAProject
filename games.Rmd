...
#  train_data <- teams_winners[-idx, ]
#  test_data  <- teams_winners[idx, ]
#  fit <- loess(point_diff ~ fieldGoalsPercentage + assists + reboundsTotal + #pointsInThePaint, data = train_data)
#  preds <- predict(fit, newdata = test_data)
#  sqrt(mean((test_data$point_diff - preds)^2, na.rm = TRUE))
#})
...
# Summary table
results <- data.frame(
  Model    = c("Linear (deg 1)", "Quadratic (deg 2)", "Cubic (deg 3)", "GAM"),
  RMSE_CV  = c(mean(cv_poly1_rmse), mean(cv_poly2_rmse), mean(cv_poly3_rmse),
              mean(cv_gam_rmse)),
  SD_CV    = c(sd(cv_poly1_rmse), sd(cv_poly2_rmse), sd(cv_poly3_rmse),
              sd(cv_gam_rmse))
) %>%
...
caretTree <- train(point_diff ~
  ...
  method = "rpart",
  trControl = trctrl,
  tuneGrid = expand.grid(cp = seq(0.008, 0.2, 0.001))
)
...
```{r}
m_lm = lm(point_diff ~ fieldGoalsPercentage + reboundsTotal + assists + steals + threePointersPercentage, data = tree_df)
