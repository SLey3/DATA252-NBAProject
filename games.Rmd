...
cv_gam_rmse <- sapply(folds, function(idx) {
  ...
  fit <- gam(point_diff ~ fieldGoalsPercentage + assists + turnovers + reboundsTotal + steals + pointsInThePaint + freeThrowsPercentage, data = train_data)
...
caretTree  <-  train(point_diff ~
    ...
    reboundsTotal, 
                    ...
                    method = "rpart",
                    trControl=trctrl,
                    tuneGrid = expand.grid(cp = seq(0.0001, 0.05, 0.0005))
)
...
## MIN ERROR
which.min(caretTree$finalModel$cptable[,"xerror"])

## WHICH CP
caretTree$finalModel$cptable[which.min(caretTree$finalModel$cptable[,"xerror"]),"CP"]

```


```{r}
m_lm = lm(point_diff ~ fieldGoalsPercentage + reboundsTotal, data = tree_df)
